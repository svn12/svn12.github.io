...
async function Find_sheetData(shtUrl,shtName,findString){
	const Search_API_V2='https://script.google.com/macros/s/AKfycbwPX2Kg04BkLlLAc4Xpzpzow3KQVNiZZpg1symjNFFIZOGTZo_PEjK-1W3u8BF4-0Yhgw/exec';
	  ...
	  elem.map((elem, index) => {
	   ...
	   if(isValidUrl(elem)){
		   ...
		   if(Number.isFinite(elem)&&!Number.isInteger(elem)){
						cell.innerHTML = elem.toLocaleString(0,{ minimumFractionDigits: 2,maximumFractionDigits: 2});
					}else{
						if(isValidDate(elem)){
	...
	rrows.map((elem, index) => {
	  ...
	  tharr.map((ele, index) => {
	   ...
	   if(isValidUrl(elem[ele])){
		   ...
		   if(Number.isFinite(Number(elem[ele]))&&!Number.isInteger(Number(elem[ele]))){
						cell.innerHTML = Number(elem[ele]).toLocaleString(0,{ minimumFractionDigits: 2,maximumFractionDigits: 2});
					}else{
